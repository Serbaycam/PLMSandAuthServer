...
namespace AuthIdentity.Service.Services
{
	public class AuthIdentityMemberService : IAuthIdentityMemberService
	{
		...
		{
			AuthIdentityUser user = _mapper.Map<AuthIdentityUser>(authIdentityUserRegisterDto);
			IdentityResult identityResult = await _userManager.CreateAsync(user, authIdentityUserRegisterDto.Password);
